...
for (i in seq_along(ensemble.files)) {
  res = read.table(file = ensemble.files[[i]], sep = "\t", 
...
# check in case of two ensemble synergy files
# that the same perturbations were tested (and in the same order)
if (length(res.list) == 2) 
  stopifnot(all(res.list[[1]]$perturbation == res.list[[2]]$perturbation))
...
for (res in res.list) {
  stopifnot(all(observed.synergies %in% res$perturbation))
}
```
