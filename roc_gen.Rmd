  ...
  sidebarLayout(
    sidebarPanel(width = 4,
      ...
      fileInput(inputId = 'rand', label = 'Select Random Ensemble Synergies File', 
                buttonLabel = 'Browse...', placeholder = 'No file selected'),
      actionButton(inputId = 'downloadRocImage', label = 'download .svg')
      ),
    mainPanel(plotlyOutput(outputId = 'rocPlot'))
  ),
...
server = function(input, output) {
  ...
  # old-styled plot
  # Use with `plotOutput, renderPlot` instead of `plotlyOutput, renderPlotly`
  get.roc.plot.old = reactive({
    method = get.method()
    roc.stats = get.roc.stats()
  ...
  # download plotly ROC
  observeEvent(input$downloadRocImage, {
    orca(p = get.roc.plotly(), file = 'roc.svg', format = 'svg')
  })
  
   # output$downloadRocImage <- downloadHandler(
   #  filename = function() {
   #    method = get.method.reactive()
   #    paste0('roc-', method, '-', Sys.Date(), '.pdf')
   #  },
   #  content = function(file) {
   #    p = get.roc.plot.reactive()
   #    orca(p, format = 'pdf', filename)
   #  })
  
  output$rocStatsTable = DT::renderDataTable({
    roc.stats = get.roc.stats()
  }, caption = 'ROC Statistics')
}
